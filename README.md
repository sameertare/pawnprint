...
- **"Why this pairing?"** — a small ⓘ button next to every board (and bye) explains the specific match-up in plain English, SwissSys-style: each player's score entering the round (same score group vs. one having floated down), color history and the exact color-due code and decision behind who got White, whether it's a rematch or first meeting, and — if it applies — a note that the two are a marked family group paired only because no conflict-free alternative existed. Works on any round, not just the latest, and is reconstructed from the round-by-round history rather than the pairing algorithm's internal trace, so it reports the objective facts of the match-up rather than every tiebreak the algorithm weighed.
