...
- **Input:** drop PGN file(s), try the bundled sample, or fetch an account's games directly by username — from **lichess** (`GET lichess.org/api/games/user/:username`, streamed as PGN) or **chess.com** (its public "Published Data API": fetch the account's monthly archive list, then the N most recent months in parallel, concatenating each game's own `pgn` field). Neither needs auth for public games. The main player is auto-detected the same way Performance Analysis does it (most frequent name across games, with casing/"Last, First"/nickname variants folded together) — no picking required; a username fetch skips the heuristic entirely and attributes every game to that account directly.
