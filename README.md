...
- **"Why this pairing?"** — a small ⓘ button next to every board (and bye) opens a per-board diagram, not just prose: score-group chips for the two players (with a ⇣ marker if one floated down to complete an odd bracket), a color-due flow showing exactly what each player was due and whether they got it, and badges for rematch/first-meeting and — if it applies — a marked family group paired only because no conflict-free alternative existed. A bullet-point summary underneath states the same facts in plain English. Works on any round, not just the latest, and is reconstructed from the round-by-round history rather than the pairing algorithm's internal trace, so it reports the objective facts of the match-up rather than every tiebreak the algorithm weighed.
