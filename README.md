...
- **📷 Read results from photo:** on the round currently being scored, upload a photo of the paper results sheet instead of entering each board by hand. Runs OCR entirely in your browser via [Tesseract.js](https://github.com/naptha/tesseract.js) (WASM; downloads its language model at runtime, but the photo itself is never uploaded anywhere) — for each board the app already paired, it looks for the line that best names both players and pulls a `1-0` / `0-1` / `½-½` token out of that line, then fills in that board's result exactly as if you'd picked it from the dropdown yourself (same `setResult()` call, so scoring/standings/next-round pairing are unaffected). Only recognizes clean, unambiguous result tokens and only fills in boards that don't already have a result — anything it can't confidently match is left for you to enter by hand, and everything it *did* fill in is still just a normal dropdown, so review and correct before pairing the next round. Best with a clearly printed or neatly handwritten one-row-per-board sheet; skewed photos, poor handwriting, or a full multi-round wall chart are a much harder ask for generic OCR and may need more manual cleanup.
...
- **🎓 Drill your repertoire:** shown only in the 🧑 My Repertoire tab, below the tree. Quizzes you on your own turn only, at every position in the tree where you've actually played a move — never on the opponent's replies. Click a piece to answer; correct means the move you played is *any* move you've actually played from that position before (a repertoire can branch — sometimes you play e4, sometimes d4, both count), not just the single most common one. A miss shows every move you've actually played there, with games/score, so you can see what you meant to play. Spaced repetition (`src/srs.ts`, a simplified SM-2 — binary correct/incorrect instead of SM-2's 0–5 quality scale, everything else the same shape: interval, ease factor, due date) schedules each position further out after a correct answer and back to daily after a miss, so weak branches surface more often without re-drilling everything you already know. A missed position also gets one more attempt later in the same session, on top of its rescheduled due date. Progress persists to `localStorage`, keyed per player *and* color, so White and Black repertoires (and different players' roster entries) are scheduled independently. "Start drilling" pulls in up to 30 due (or new) positions per session, oldest-due/never-seen first.
