...
- **"Why this pairing?"** — a small ⓘ button next to every board (and bye) explains the match-up in plain English, SwissSys-style: each player's score entering the round (same score group vs. one having floated), their color history before this round, and whether it's a rematch or first meeting. Works on any round, not just the latest, and is reconstructed from the round-by-round history rather than the pairing algorithm's internal trace, so it reports the objective facts of the match-up rather than every tiebreak the algorithm weighed.
