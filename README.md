...
- **Per-game strength/weakness assessment** — a 🩺 button on every analyzed game expands a verdict for that specific game: an overall summary (result, accuracy, and the single biggest factor — a decisive-phase mistake, a thrown win, or a resilient save), then opening/middlegame/endgame each individually marked as a strength, weakness, or neutral with a specific reason (accuracy, error counts, whether it decided the game, and for the opening, whether it's a repeated line from your repertoire or a one-off). Derived entirely from the same accuracy/error data as the rest of the report — nothing is re-analyzed. Every analyzed game gets this breakdown in the **downloaded report** too, as its own numbered section.
